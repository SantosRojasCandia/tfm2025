...
for cult in list_col:
    ...
    for i in range(serie_post.index.get_loc(inicio_decrecimiento) + 1, len(serie_post) - duracion_min):
        ...
        if (ventana > -umbral_crecimiento).all():
    ...
    ax.set_title(f"Ciclo LAI: parcela {cult} ({type_c})")
    ax.legend()
    plt.tight_layout()
...
output_csv = pth.join(out_fold, 'ciclo_LAI_completo_2022.csv')
